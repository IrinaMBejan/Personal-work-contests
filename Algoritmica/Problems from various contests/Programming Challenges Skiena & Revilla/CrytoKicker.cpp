...
bool MapWord(string DWord, string TWord)
{
    ...
    for (int j=0; j<2; j++)
        for (int i='a'; i<='z'; i++)
            OriginalMappings[j][i] = Mappings[j][i];
...
void outputModified(vector<string> splitTokens)
{
    ...
    {
        for (int j=0; j<(int)splitTokens[i].length(); j++)
            cout << (char)Mappings[1][splitTokens[i][j]];
...
int main()
{
    ...
    while(fgets(input, 93,stdin)!= NULL)
    {
        ...
        for (int i=0; i< splitTokens.size(); i++)
            sortedSplitTokens.push_back(splitTokens[i]);
